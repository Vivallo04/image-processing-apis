...
int ImageController::GaussianBlur_Filter(const cv::Mat &img_orig, cv::Mat &img_new, int width, int height)
{
    if(img_orig.empty())
    {
...
int ImageController::GrayScale_Filter(const cv::Mat &img_orig, cv::Mat &img_new, int width, int height) {
    cv::Mat img_sizer;
    cv::Mat();
    if(img_orig.empty())
    {
...
int ImageController::BrightnessControl_Filter(const cv::Mat &img_orig, cv::Mat &img_new, int delta_bright, int width,
                                                  int height) {
    cv::Mat img_sizer;
    if(img_orig.empty())
    {
...
int ImageController::GammaCorrection_Filter(const cv::Mat &img_orig, cv::Mat &img_new, int gamma_cor, int width,int height) {
    {
        cv::Mat img_sizer;
